...
from . import prompts_v1_27 as prompts
from .api_model_utils import parse_api_model_id
from .utils_v1_27 import load_vl_model, ToolCallingShim, QwenVLEmbeddings, TextModelEmbeddings
...
class Context:
    ...
    counterfactual_template_num: int = field(
        default=1,
        metadata={
            "description": "Number of counterfactual template reports to generate. Default: 1."
        },
    )

    counterfactual_strictness: list[str] = field(
        default_factory=lambda: ["soft"],
        metadata={
            "description": "Per-template strictness list. Supported values: soft, hard. "
            "Length must match counterfactual_template_num."
        },
    )

    counterfactual_template_source: list[str] = field(
        default_factory=lambda: ["none"],
        metadata={
            "description": "Per-template source list. Use none for normal atomic candidates and "
            "rule_file/external/prepared for prepared logical rule templates."
        },
    )

    counterfactual_matching_rule: list[str] = field(
        default_factory=lambda: ["textual_matching"],
        metadata={
            "description": "Per-template matching rule list. Supported values in v1_27: "
            "textual_matching and visual_check."
        },
    )

    counterfactual_rules_file: Optional[str] = field(
        default=None,
        metadata={
            "description": "Path to a prepared rules file for visual_check templates. "
            "Supports JSON keyed by class name or plain text sections like 'Class Name: rule text'."
        },
    )

    counterfactual_checks_file: Optional[str] = field(
        default=None,
        metadata={
            "description": "Path to fixed atomic visual checks generated from rule_text before image processing. "
            "If unset, the default path is next to atomic_candidates_file."
        },
    )

    hard_rule_fail_confidence: float = field(
        default=0.7,
        metadata={
            "description": "Minimum confidence required for a hard visual_check fail to count as a hard rule violation."
        },
    )

    hard_rule_min_fail_count: int = field(
        default=1,
        metadata={
            "description": "Minimum number of high-confidence fail checks required to trigger hard_rule_violation."
        },
    )
