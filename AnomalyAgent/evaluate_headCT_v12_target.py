"""
Evaluate agent_v1_16 graph on HeadCT dataset using WinCLIP evaluation metrics.

This script:
1. Loads HeadCT test dataset
2. Runs agent_v1_16 graph on each image
...
DEFAULT_HEADCT_DATA_PATH = os.environ.get("HEADCT_DATA_PATH", "./data/headct/images")
DEFAULT_HEADCT_LABELS_CSV = os.environ.get("HEADCT_LABELS_CSV", "./data/headct/labels.csv")
...
HEADCT_CLASS_NAME = "brain CT image"
HEADCT_TARGET_ANOMALY_DEFINITION = (
    "Only intracranial hemorrhage or visible bleeding is anomalous. "
    "Head CT images without hemorrhage should be judged normal/target-negative, "
    "even if other non-hemorrhagic findings are visible."
)
TARGET_PROMPT4CANDIDATE_GENERATION = """You are generating atomic candidate descriptions for targeted anomaly detection.

Given:
- Class name: {class_name}
- Target anomaly definition: {target_anomaly_definition}

Generate two candidate sets in strict JSON format:

1. anomaly_candidates: 10-12 short phrases or sentences describing ONLY target-positive cases under the target anomaly definition.

2. normal_candidates: The same quantity as anomaly_candidates, describing target-negative cases for this class.

Guidelines:
- Each candidate MUST be a short phrase or sentence (1-10 words).
- anomaly_candidates MUST match the target anomaly definition.
- normal_candidates should describe absence of the target anomaly and may include non-target abnormalities if they are target-negative.
- Do NOT include non-target abnormalities in anomaly_candidates.
- Do NOT use generic words like "defective", "damaged", or "abnormal" alone; always be specific.
- Do NOT treat synonyms as different candidates; avoid pseudo-diversity.
...
def _first_existing_column(row: Dict, candidates: List[str]) -> Optional[str]:
    """Return the first matching CSV column name, case-insensitively."""
    lower_to_original = {str(k).strip().lower(): k for k in row.keys()}
...
def _normalize_id_value(value) -> str:
    """Normalize CSV id values like 0, '0', or '0.0' to a stable string."""
    value_text = str(value).strip()
    if not value_text:
        raise ValueError("Empty id value in labels CSV")
...
def _candidate_id_keys(value) -> List[str]:
    """Return lookup keys for an id or filename, including common zero-padded forms."""
    normalized = _normalize_id_value(value)
    path = Path(normalized)
    base_values = {normalized, path.name}

    if path.suffix:
        base_values.add(path.stem)

    numeric_values = set()
    for base in list(base_values):
        try:
            numeric_value = int(float(base))
        except ValueError:
            continue
        numeric_values.add(str(numeric_value))
        for width in (3, 4, 5, 6):
            numeric_values.add(f"{numeric_value:0{width}d}")

    keys = set()
    for base in base_values | numeric_values:
        keys.add(str(base).lower())
        if not Path(str(base)).suffix:
            for ext in IMAGE_EXTENSIONS:
                keys.add(f"{base}{ext}".lower())

    return sorted(keys)


def _build_image_index(image_root: Path) -> Dict[str, Path]:
    """Index image files by relative path, filename, stem, id, and zero-padded id."""
    ...
    for image_path in sorted(image_root.rglob("*")):
        ...
        relative_path = str(image_path.relative_to(image_root)).lower()
        direct_keys = {
            relative_path,
        ...
        for key in _candidate_id_keys(image_path.stem):
            image_index.setdefault(key, image_path)

    return image_index


def _label_to_anomaly(value, label_column: str) -> int:
    """Map common HeadCT label values to anomaly=1, normal=0."""
    value_text = str(value).strip().lower()
    column_text = str(label_column).strip().lower()
    ...
    if value_text in positive_values:
        anomaly = 1
    elif value_text in negative_values:
        anomaly = 0
    else:
        try:
            anomaly = 1 if float(value_text) > 0 else 0
        except ValueError as exc:
            raise ValueError(f"Cannot parse label value '{value}' in column '{label_column}'") from exc

    # If the only available label column explicitly encodes normality, invert it.
    if column_text in {"normal", "is_normal", "healthy", "is_healthy"}:
        anomaly = 1 - anomaly

    return anomaly


def _resolve_image_path(image_root: Path, raw_id: str, image_index: Dict[str, Path]) -> Path:
    """Resolve CSV id values to image files named by id."""
    ...
    direct_candidate = image_root / path
    if direct_candidate.is_file():
        return direct_candidate

    for key in _candidate_id_keys(raw_id):
        image_path = image_index.get(key)
        if image_path is not None:
            return image_path

    # Return the most likely path so the error message remains useful.
    return image_root / f"{_normalize_id_value(raw_id)}.png"


def load_headct_test_data(data_path: str, labels_csv: str) -> List[Dict]:
    """Load HeadCT test data from labels.csv with id/hemorrhage columns.

    Args:
        data_path: Directory containing HeadCT image files named by id.
        labels_csv: CSV file containing id and hemorrhage labels.

    Returns:
        List of test samples, each containing img_path, cls_name, anomaly label.
    """
    root = Path(data_path)
    labels_path = Path(labels_csv)

    if not root.is_dir():
        raise FileNotFoundError(f"HeadCT image directory not found: {root}")
    if not labels_path.is_file():
        raise FileNotFoundError(f"HeadCT labels CSV not found: {labels_path}")

    test_data = []
    with open(labels_path, "r", encoding="utf-8-sig", newline="") as f:
        reader = csv.DictReader(f)
        if reader.fieldnames is None:
            raise ValueError(f"Labels CSV has no header: {labels_path}")
        ...
        if id_column is None:
            raise ValueError(
                "Could not find an id column in labels CSV. "
                f"Available columns: {reader.fieldnames}"
            )
        if label_column is None:
            raise ValueError(
                "Could not find a label column in labels CSV. "
                f"Available columns: {reader.fieldnames}"
            )

        image_index = _build_image_index(root)
        if not image_index:
            raise FileNotFoundError(f"No image files found under HeadCT image directory: {root}")

        for row in rows:
            image_id = _normalize_id_value(row[id_column])
            img_path = _resolve_image_path(root, image_id, image_index)
            if not img_path.is_file():
                raise FileNotFoundError(
                    f"Image referenced by labels CSV id was not found: {row[id_column]} "
                    f"(resolved as {img_path})"
                )
            anomaly = _label_to_anomaly(row[label_column], label_column)
            test_data.append(
                {
                    ...
                    "img_path": (
                        ...
                        if img_path.is_relative_to(root)
                        else str(img_path)
                    ),
                    "cls_name": HEADCT_CLASS_NAME,
                    "anomaly": anomaly,
                    "id": image_id,
...
async def generate_and_save_target_atomic_candidates(
    object_names: List[str],
    context: Context,
    output_file: str,
    target_anomaly_definition: str,
    file_name: Optional[str] = None,
) -> Dict[str, Dict[str, Any]]:
    """Generate target-aware atomic candidates and save them to a JSON file."""
    ...
    if file_name is not None:
        ...
        if file_path.exists() and file_path.is_file():
            try:
                ...
                print(f"Using existing target candidate set from: {file_name}")
                return all_candidates
            except Exception as e:
                print(f"Warning: Failed to load existing target candidate set from {file_name}: {e}")
                print("Proceeding with generation...")

    model = context.atomic_candidate_llm_model
    target_anomaly_definition = target_anomaly_definition.strip()
    if not target_anomaly_definition:
        raise ValueError("target_anomaly_definition must be non-empty for target-aware candidate generation.")

    if not hasattr(context, "_atomic_candidate_cache"):
        context._atomic_candidate_cache = {}
    ...
    def _get_text(content) -> str:
        ...
        def _collect_text_fields(value) -> None:
            if isinstance(value, dict):
                text_value = value.get("text")
                if isinstance(text_value, str) and text_value:
                    fragments.append(text_value)
                for child in value.values():
                    _collect_text_fields(child)
            elif isinstance(value, list):
                for child in value:
                    _collect_text_fields(child)

        _collect_text_fields(content)
        return "\n".join(fragment for fragment in fragments if fragment)

    prompt_hash = hashlib.md5(
        f"{TARGET_PROMPT4CANDIDATE_GENERATION}\n{target_anomaly_definition}".encode()
    ).hexdigest()[:8]
    try:
    ...
    target_definition_hash = hashlib.md5(target_anomaly_definition.encode()).hexdigest()[:8]

    all_candidates: Dict[str, Dict[str, Any]] = {}
    pbar = tqdm(object_names, desc="Generating target atomic candidates", unit="object")
    for class_name in pbar:
        ...
        pbar.set_description(f"Generating target atomic candidates: {class_name}")

        try:
            candidate_cache_key = (
                f"{standardized_class_name}_{target_definition_hash}_{prompt_hash}_{model_fingerprint_hash}"
            )
            if candidate_cache_key in context._atomic_candidate_cache:
                anomaly_candidates, normal_candidates, candidate_version = context._atomic_candidate_cache[
                    candidate_cache_key
                ]
                tqdm.write(f"  Using cached target candidates (version: {candidate_version})")
            else:
                prompt = TARGET_PROMPT4CANDIDATE_GENERATION.format(
                    class_name=class_name,
                    target_anomaly_definition=target_anomaly_definition,
                )
                ...
                try:
                    if hasattr(model, "bind"):
                        ...
                        resp = await bound_model.ainvoke(msg_list)
                ...
                anomaly_candidates = [
                    c for c in anomaly_candidates if c and isinstance(c, str) and c.strip()
                ]
                normal_candidates = [
                    c for c in normal_candidates if c and isinstance(c, str) and c.strip()
                ]

                candidate_content = json.dumps(
                    [target_anomaly_definition, anomaly_candidates, normal_candidates],
                    sort_keys=True,
                )
                candidate_version = hashlib.md5(candidate_content.encode()).hexdigest()[:8]
                context._atomic_candidate_cache[candidate_cache_key] = (
                    anomaly_candidates,
                    normal_candidates,
                    candidate_version,
                )
                ...
                tqdm.write(
                    f"  Generated new target candidates "
                    f"(version: {candidate_version}, temperature: {temp_status})"
                )
            ...
            tqdm.write(
                f"  Result: {len(anomaly_candidates)} anomaly candidates and "
                f"{len(normal_candidates)} normal candidates"
            )
        except Exception as e:
            tqdm.write(f"Error generating target candidates for {class_name}: {e}")
            all_candidates[class_name] = {
                "target_anomaly_definition": target_anomaly_definition,
                "target_definition_hash": target_definition_hash,
                "anomaly_candidates": [],
    ...
    print(f"\nSaved target atomic candidates to: {output_path}")
    print(f"Total objects processed: {len(all_candidates)}")
    return all_candidates


def limit_samples_balanced(test_data: List[Dict], max_samples: Optional[int]) -> List[Dict]:
    """Limit samples while keeping both normal and anomaly labels when possible."""
    if max_samples is None or max_samples <= 0 or len(test_data) <= max_samples:
        return test_data

    by_label = {
        0: [sample for sample in test_data if sample.get("anomaly") == 0],
        1: [sample for sample in test_data if sample.get("anomaly") == 1],
    }
    limited = []
    indices = {0: 0, 1: 0}

    while len(limited) < max_samples and (
        indices[0] < len(by_label[0]) or indices[1] < len(by_label[1])
    ):
        for label in (0, 1):
            if len(limited) >= max_samples:
                break
            if indices[label] < len(by_label[label]):
                limited.append(by_label[label][indices[label]])
                indices[label] += 1
...
async def run_agent_on_image(
    ...
    class_name: str,
    context: Context,
    target_anomaly_definition: Optional[str] = None,
    ...
    prompt_text = f"Please analyze this {class_name} for anomalies."
    if target_anomaly_definition:
        prompt_text += f" For this evaluation, anomalous means: {target_anomaly_definition}"
    ...
    try:
        ...
        try:
            ...
            user_message = HumanMessage(
                content=f"Please analyze this {class_name} located at {image_path} for anomalies. "
                f"The image shows a {class_name} that may or may not contain abnormalities. "
                f"For this evaluation, anomalous means: {target_anomaly_definition or 'the target abnormality'}"
            )
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="Evaluate agent_v1_16 on HeadCT dataset")
    parser.add_argument(
        "--data_path",
        type=str,
        default=DEFAULT_HEADCT_DATA_PATH,
        help="Path to HeadCT image directory. Images are expected to be named by labels.csv id.",
    )
    parser.add_argument(
        "--labels_csv",
        type=str,
        default=DEFAULT_HEADCT_LABELS_CSV,
        help="Path to HeadCT labels CSV with id and hemorrhage columns",
    )
    parser.add_argument(
        "--save_path",
        type=str,
        default="./results/headCT_agent_v1_16_target_eval",
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Name of existing target atomic candidates file in save_path directory. If None (default), will generate target-aware candidates at runtime.",
    )
    parser.add_argument(
        "--target_anomaly_definition",
        type=str,
        default=HEADCT_TARGET_ANOMALY_DEFINITION,
        help="Task definition for target-positive anomalies. Default: only intracranial hemorrhage/bleeding is anomalous.",
    )
    parser.add_argument(
    ...
    # Load test data
    print(f"Loading HeadCT test data from {args.data_path}...")
    print(f"Loading HeadCT labels from {args.labels_csv}...")
    test_data = load_headct_test_data(args.data_path, args.labels_csv)

    if args.max_samples:
        test_data = limit_samples_balanced(test_data, args.max_samples)

    print(f"Loaded {len(test_data)} test samples")
    normal_count = sum(1 for sample in test_data if sample.get("anomaly") == 0)
    anomaly_count = sum(1 for sample in test_data if sample.get("anomaly") == 1)
    print(f"Label counts: normal={normal_count}, anomaly={anomaly_count}")
    if args.resize:
        print("Resize preprocessing enabled: all test images will be resized to 518x518.")

    print(f"Target anomaly definition: {args.target_anomaly_definition}")

    # HeadCT has a single semantic class, so atomic candidates are generated
    # only once for this class and every test sample uses the same cls_name.
    obj_list = [HEADCT_CLASS_NAME]
    print(f"Using HeadCT class for atomic candidates: {obj_list}")
    ...
    # If user provided --atomic_candidates_file, use that; otherwise write HeadCT target candidates.
    ...
    if args.atomic_candidates_file is not None:
        ...
        atomic_candidates_file = os.path.abspath(os.path.join(args.save_path, "atomic_candidates_headct_target.json"))
    context = Context(
    ...
    _ = context.atomic_candidate_llm_model  # Preload target candidate generation model
    print("Models preloaded successfully.")
    
    # Generate/load target-aware atomic candidates for all classes.
    # Note: atomic_candidates_file path is already set in context above
    if args.atomic_candidates_file is None:
        print("\nGenerating HeadCT target-aware atomic candidates...")
        try:
            await generate_and_save_target_atomic_candidates(
                object_names=obj_list,
                context=context,
                output_file=atomic_candidates_file,
                target_anomaly_definition=args.target_anomaly_definition,
            )
            print(f"HeadCT target atomic candidates saved to: {atomic_candidates_file}")
        except Exception as e:
            print(f"Warning: Failed to generate HeadCT target atomic candidates: {e}")
            print("Continuing with evaluation...")
    else:
        # Use existing file
        print(f"\nUsing existing target atomic candidates file: {args.atomic_candidates_file}")
        try:
            await generate_and_save_target_atomic_candidates(
                object_names=obj_list,
                context=context,
            ...
            print(f"Warning: Failed to load target atomic candidates from {args.atomic_candidates_file}: {e}")
            print("Continuing with evaluation...")
...
# toy test: python evaluate_headCT_v12_target.py --data_path ./data/headct/images --labels_csv ./data/headct/labels.csv --save_path ./results/headCT_target_test --max_samples 10 --qwen_model_path ./models/qwen3_vl_4b --device cuda --concurrent 2 --uncertain_score 0.5 --max_batch_size 4 --max_wait_ms 50 --show_token_num
# toy test: python evaluate_headCT_v12_target.py --data_path ./data/headct/images --labels_csv ./data/headct/labels.csv --save_path ./results/headCT_target_test --max_samples 10 --qwen_model_path ./models/qwen3_vl_4b --device cuda --concurrent 2 --uncertain_score 0.5 --max_batch_size 4 --max_wait_ms 50 --show_token_num --atomic_candidates_file atomic_candidates_headct_target.json
