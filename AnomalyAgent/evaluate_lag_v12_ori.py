"""
Evaluate agent_v1_16 graph on LAG dataset using WinCLIP evaluation metrics.

This script:
1. Loads LAG test dataset
...
DEFAULT_LAG_DATA_PATH = os.environ.get("LAG_DATA_PATH", "./data/lag/test")
...
def load_lag_test_data(data_path: str) -> List[Dict]:
    """Load LAG test data from good/defect folders.

    Args:
        data_path: LAG test directory containing good/ and defect/ folders.
    ...
    if not root.is_dir():
        raise FileNotFoundError(f"LAG test directory not found: {root}")

    missing_dirs = [str(p) for p in (good_dir, defect_dir) if not p.is_dir()]
    if missing_dirs:
        raise FileNotFoundError(
            "LAG test directory must contain 'good' and 'defect' folders. "
            f"Missing: {missing_dirs}"
        )

    def iter_images(label_dir: Path):
        return sorted(
            p
    ...
    test_data = []
    for label_dir, anomaly in ((good_dir, 0), (defect_dir, 1)):
...
async def run_agent_on_image(
    ...
    try:
        ...
        user_message = HumanMessage(
            content=[
                {"type": "text", "text": f"Please analyze this {class_name} for anomalies."},
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="Evaluate agent_v1_16 on LAG dataset")
    parser.add_argument(
        "--data_path",
        type=str,
        default=DEFAULT_LAG_DATA_PATH,
        help="Path to LAG test directory containing good/ and defect/ folders.",
    )
    parser.add_argument(
        "--save_path",
        type=str,
        default="./results/lag_agent_v1_16_eval",
    ...
    # Load test data
    print(f"Loading LAG test data from {args.data_path}...")
    test_data = load_lag_test_data(args.data_path)
    ...
    # LAG has a single semantic class, so atomic candidates are generated
    # only once for this class and every test sample uses the same cls_name.
    obj_list = [LAG_CLASS_NAME]
    print(f"Using LAG class for atomic candidates: {obj_list}")
...
# toy test: python evaluate_lag_v12_ori.py --data_path ./data/lag/test --save_path ./results/lag_ori_test --max_samples 10 --qwen_model_path ./models/qwen3_vl_4b --device cuda --concurrent 2 --uncertain_score 0.5 --max_batch_size 4 --max_wait_ms 50 --show_token_num
# toy test: python evaluate_lag_v12_ori.py --data_path ./data/lag/test --save_path ./results/lag_ori_test --max_samples 10 --qwen_model_path ./models/qwen3_vl_4b --device cuda --concurrent 2 --uncertain_score 0.5 --max_batch_size 4 --max_wait_ms 50 --show_token_num --atomic_candidates_file atomic_candidates.json
