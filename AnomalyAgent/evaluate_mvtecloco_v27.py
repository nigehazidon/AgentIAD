"""
Evaluate agent_v1_27 graph on MVTec LOCO dataset using WinCLIP evaluation metrics.

This script:
1. Loads MVTec LOCO test dataset
2. Runs agent_v1_27 graph on each image
...
DEFAULT_MVTEC_LOCO_DATA_PATH = os.environ.get("MVTEC_LOCO_DATA_PATH", "./data/mvtec_loco")
DEFAULT_QWEN_VL_MODEL_PATH = os.environ.get("QWEN_VL_MODEL_PATH", "./models/qwen3_vl_4b")

# Add project root to path
sys.path.insert(0, str(Path(__file__).parent / "src"))

# === Import v1_27 agent graph and its heuristic ===
from react_agent.agent_v1_27 import graph, _keyword_heuristic_decision
from react_agent.context_v1_27 import Context
from react_agent.state_v1_27 import InputState, State
from react_agent.tools_v1_27 import generate_and_save_counterfactual_atomic_visual_checks
from react_agent.utils_v1_27 import generate_and_save_atomic_candidates
from utils.image_utils import resize_image
# from react_agent.utils_v1_27 import extract_reasoning_timeline_from_state
...
def load_mvtecloco_test_data(data_path: str) -> List[Dict]:
    """Load MVTec LOCO test dataset from class/test subdirectories.

    Args:
        data_path: Path to MVTec LOCO root directory.
    ...
    if not data_root.exists():
        raise FileNotFoundError(f"MVTec LOCO data root does not exist: {data_root}")

    missing_dirs = []
    test_data = []

    for cls_name in MVTEC_LOCO_CLASSES:
        test_root = data_root / cls_name / "test"
        if not test_root.exists():
            missing_dirs.append(str(test_root))
            continue

        for anomaly_type, anomaly_label in MVTEC_LOCO_TEST_SPLITS.items():
            split_dir = test_root / anomaly_type
            if not split_dir.exists():
                missing_dirs.append(str(split_dir))
                continue
                ...
                test_data.append(
                    {
                        ...
                        "full_mask_path": None,
                        "img_path": str(image_path.relative_to(data_root)),
                        "cls_name": cls_name,
    ...
    if missing_dirs:
        print(
            "[WARNING] Missing MVTec LOCO directories were skipped:\n"
            + "\n".join(f"  - {path}" for path in missing_dirs)
        )

    if not test_data:
        expected = ", ".join(MVTEC_LOCO_TEST_SPLITS.keys())
        raise ValueError(
            f"No MVTec LOCO test images found under {data_root}. "
            f"Expected <object>/test/{{{expected}}}/*.png for objects: {MVTEC_LOCO_CLASSES}"
        )

    return test_data


def _parse_counterfactual_list_arg(value: Any, default: str, expected_len: int, arg_name: str) -> List[str]:
    """Normalize argparse list/comma/JSON-list values and enforce expected length."""
    if value is None:
        items = [default]
        ...
        if text.startswith("["):
            ...
            if not isinstance(parsed, list):
                raise ValueError(f"{arg_name} JSON value must be a list.")
    ...
    if len(items) != expected_len:
        raise ValueError(
            f"{arg_name} length ({len(items)}) must match --counterfactual_template_num ({expected_len})."
        )
    return items
    ...
    try:
        ...
        try:
            ...
            if not isinstance(final_result, str) or final_result_lower not in valid_labels or final_result_lower == "uncertain":
                ...
                # Use the same heuristic as the agent.
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="Evaluate agent_v1_27 on MVTec LOCO dataset")
    parser.add_argument(
        "--data_path",
        type=str,
        default=DEFAULT_MVTEC_LOCO_DATA_PATH,
        help="Path to MVTec LOCO dataset root",
    )
    parser.add_argument(
        "--save_path",
        type=str,
        default="./results/mvtecloco_agent_v1_27_eval",
    ...
    parser.add_argument(
        "--counterfactual_template_num",
        type=int,
        default=1,
        help="Number of counterfactual template reports to generate (default: 1).",
    )
    parser.add_argument(
        "--counterfactual_strictness",
        type=str,
        nargs="+",
        default=["soft"],
        help="Per-template strictness list. Values: soft or hard. Length must match --counterfactual_template_num.",
    )
    parser.add_argument(
        "--counterfactual_template_source",
        type=str,
        nargs="+",
        default=["none"],
        help="Per-template source list. Use none for normal candidates and rule_file/external/prepared for prepared rule files.",
    )
    parser.add_argument(
        "--counterfactual_matching_rule",
        type=str,
        nargs="+",
        default=["textual_matching"],
        help="Per-template matching list. Supported in v1_27: textual_matching or visual_check.",
    )
    parser.add_argument(
        "--counterfactual_rules_file",
        type=str,
        default=None,
        help=(
            "Prepared rules file for visual_check templates. Supports JSON or plain text sections like "
            "'Splicing Connectors: Exactly two ...'. Relative paths are resolved under save_path."
        ),
    )
    parser.add_argument(
        "--counterfactual_checks_file",
        type=str,
        default="counterfactual_atomic_visual_checks.json",
        help=(
            "Output file for fixed atomic visual checks generated under save_path before image processing. "
            "Default: counterfactual_atomic_visual_checks.json."
        ),
    )
    parser.add_argument(
        "--hard_rule_fail_confidence",
        type=float,
        default=0.7,
        help="Minimum confidence for a hard visual_check fail to trigger hard_rule_violation logic (default: 0.7).",
    )
    parser.add_argument(
        "--hard_rule_min_fail_count",
        type=int,
        default=1,
        help="Minimum number of high-confidence hard visual_check fails required for hard_rule_violation (default: 1).",
    )
    parser.add_argument(
    ...
    if args.counterfactual_template_num < 1:
        parser.error("--counterfactual_template_num must be >= 1")
    try:
        args.counterfactual_strictness = _parse_counterfactual_list_arg(
            args.counterfactual_strictness,
            "soft",
            args.counterfactual_template_num,
            "--counterfactual_strictness",
        )
        args.counterfactual_template_source = _parse_counterfactual_list_arg(
            args.counterfactual_template_source,
            "none",
            args.counterfactual_template_num,
            "--counterfactual_template_source",
        )
        args.counterfactual_matching_rule = _parse_counterfactual_list_arg(
            args.counterfactual_matching_rule,
            "textual_matching",
            args.counterfactual_template_num,
            "--counterfactual_matching_rule",
        )
    ...
    normalized_strictness = [item.strip().lower() for item in args.counterfactual_strictness]
    normalized_matching = [item.strip().lower().replace("-", "_") for item in args.counterfactual_matching_rule]
    for item in normalized_strictness:
        if item not in {"soft", "hard"}:
            parser.error(f"Unsupported --counterfactual_strictness value: {item}")
    for item in normalized_matching:
        if item in {"textual", "text", "embedding", "caption_candidate"}:
            item = "textual_matching"
        elif item in {"visual", "rule_check", "visual_rule_check"}:
            item = "visual_check"
        if item not in {"textual_matching", "visual_check"}:
            parser.error(f"Unsupported --counterfactual_matching_rule value: {item}")
    args.counterfactual_strictness = normalized_strictness
    args.counterfactual_matching_rule = [
        "textual_matching"
        if item in {"textual", "text", "embedding", "caption_candidate"}
        else "visual_check"
        if item in {"visual", "rule_check", "visual_rule_check"}
        else item
        for item in normalized_matching
    ]
    if any(
        item in {"visual_check", "visual", "rule_check", "visual_rule_check"}
        for item in normalized_matching
    ) and args.counterfactual_rules_file is None:
        parser.error("--counterfactual_rules_file is required when any matching rule is visual_check")
    if not 0.0 <= args.hard_rule_fail_confidence <= 1.0:
        parser.error("--hard_rule_fail_confidence must be in [0, 1]")
    if args.hard_rule_min_fail_count < 1:
        parser.error("--hard_rule_min_fail_count must be >= 1")
    ...
    test_data = load_mvtecloco_test_data(args.data_path)
    ...
    obj_list = MVTEC_LOCO_CLASSES
    print(f"Using {len(obj_list)} classes: {obj_list}")
    print(
        "Counterfactual templates: "
        f"num={args.counterfactual_template_num}, "
        f"strictness={args.counterfactual_strictness}, "
        f"source={args.counterfactual_template_source}, "
        f"matching={args.counterfactual_matching_rule}, "
        f"rules_file={args.counterfactual_rules_file}, "
        f"checks_file={args.counterfactual_checks_file}, "
        f"hard_rule_fail_confidence={args.hard_rule_fail_confidence}, "
        f"hard_rule_min_fail_count={args.hard_rule_min_fail_count}"
    )

    # Filter samples by class (defensive)
    filtered_samples = [sample for sample in test_data if sample.get("cls_name") in obj_list]

    # Initialize context (must match agent_v1_27.Context)
    ...
    counterfactual_rules_file = (
        os.path.abspath(os.path.join(args.save_path, args.counterfactual_rules_file))
        if args.counterfactual_rules_file is not None
        else None
    )
    counterfactual_checks_file = (
        os.path.abspath(os.path.join(args.save_path, args.counterfactual_checks_file))
        if args.counterfactual_checks_file is not None
        else None
    )
    context = Context(
        ...
        counterfactual_template_num=args.counterfactual_template_num,
        counterfactual_strictness=args.counterfactual_strictness,
        counterfactual_template_source=args.counterfactual_template_source,
        counterfactual_matching_rule=args.counterfactual_matching_rule,
        counterfactual_rules_file=counterfactual_rules_file,
        counterfactual_checks_file=counterfactual_checks_file,
        hard_rule_fail_confidence=args.hard_rule_fail_confidence,
        hard_rule_min_fail_count=args.hard_rule_min_fail_count,
    ...
    if any(item == "visual_check" for item in args.counterfactual_matching_rule):
        print(f"\nGenerating fixed counterfactual atomic visual checks for {len(obj_list)} classes...")
        try:
            await generate_and_save_counterfactual_atomic_visual_checks(
                object_names=obj_list,
                context=context,
                output_file=counterfactual_checks_file,
                rules_file=counterfactual_rules_file,
            )
            print(f"Fixed counterfactual atomic visual checks ready: {counterfactual_checks_file}")
        except Exception as e:
            raise RuntimeError(
                "Failed to generate fixed counterfactual atomic visual checks before image processing."
            ) from e
...
# toy test: python evaluate_mvtecloco_v27.py --data_path ./data/mvtec_loco --save_path ./results/mvtecloco_test --max_samples 10 --qwen_model_path ./models/qwen3_vl_4b --device cuda --concurrent 2 --uncertain_score 0.5 --max_batch_size 4 --max_wait_ms 50 --show_token_num
# toy test: python evaluate_mvtecloco_v27.py --data_path ./data/mvtec_loco --save_path ./results/mvtecloco_test --max_samples 10 --qwen_model_path ./models/qwen3_vl_4b --device cuda --concurrent 2 --uncertain_score 0.5 --max_batch_size 4 --max_wait_ms 50 --show_token_num --atomic_candidates_file atomic_candidates.json
