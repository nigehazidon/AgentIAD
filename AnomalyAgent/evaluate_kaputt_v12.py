"""
Evaluate agent_v1 graph on Kaputt dataset using WinCLIP evaluation metrics.

This script:
1. Loads Kaputt test dataset from parquet metadata
...
DEFAULT_KAPUTT_PARQUET_PATH = os.environ.get("KAPUTT_PARQUET_PATH", "./data/kaputt/query-test.parquet")
DEFAULT_KAPUTT_CROP_ROOT = os.environ.get("KAPUTT_CROP_ROOT", "./data/kaputt/query-crop/data/test/query-data/crop")
DEFAULT_KAPUTT_QUERY_IMAGE_ROOT = os.environ.get("KAPUTT_QUERY_IMAGE_ROOT", "./data/kaputt/query-image/data/test/query-data")
DEFAULT_QWEN_VL_MODEL_PATH = os.environ.get("QWEN_VL_MODEL_PATH", "./models/qwen3_vl_4b")

# Add project root to path
sys.path.insert(0, str(Path(__file__).parent / "src"))

# === Import agent_v1 graph and its heuristic ===
...
def load_kaputt_test_data(
    ...
    """Load Kaputt test dataset from parquet metadata.

    Args:
        parquet_path: Path to Kaputt parquet metadata file.
        crop_root: Root directory for cropped images.
        query_image_root: Root directory for full query images (overrides parquet paths).
        mode: Evaluation mode - "any" (undamaged vs damaged) or "major" (undamaged vs major only).
        image_subdir: Subdirectory under crop_root, if any.

    Returns:
        (test_data, obj_list) where:
        - test_data: list of samples with image paths and labels.
        - obj_list: list of item_material classes.
    """
    df = pd.read_parquet(parquet_path)

    if mode == "major":
        # Exclude minor defects (defect=True, major_defect=False)
        df = df[~((df["defect"] == True) & (df["major_defect"] == False))]
        df = df.reset_index(drop=True)

    test_data = []
    for _, row in df.iterrows():
        query_image_rel = row["query_image"]
        filename = os.path.basename(query_image_rel)

        if image_subdir:
            crop_image_path = os.path.join(crop_root, image_subdir, filename)
        else:
            crop_image_path = os.path.join(crop_root, filename)

        # Always resolve full query image path using user-provided root
        full_query_image_path = os.path.join(query_image_root, query_image_rel)

        if mode == "any":
            anomaly = 1 if bool(row["defect"]) else 0
        else:
            anomaly = 1 if bool(row["major_defect"]) else 0

        test_data.append(
            {
                "full_img_path": crop_image_path,
                "full_query_image_path": full_query_image_path,
                "img_path": os.path.relpath(crop_image_path, crop_root),
                "cls_name": row["item_material"],
                "anomaly": anomaly,
                "capture_id": row["capture_id"],
                "item_identifier": row["item_identifier"],
                "defect": bool(row["defect"]),
                "major_defect": bool(row["major_defect"]),
                "defect_types": row["defect_types"] if pd.notna(row["defect_types"]) else "",
                "query_image": query_image_rel,
            }
        )

    obj_list = sorted(df["item_material"].unique().tolist())
    return test_data, obj_list
    ...
    try:
        ...
        try:
            ...
            if not isinstance(final_result, str) or final_result_lower not in valid_labels or final_result_lower == "uncertain":
                ...
                # Use the **same** heuristic as the agent (imported from agent_v1_16)
...
async def evaluate_mode(args, mode: str) -> None:
    """Evaluate one mode (any or major)."""
    # Create output directory
    os.makedirs(args.save_path, exist_ok=True)

    # Load test data
    print(f"Loading test data from {args.parquet_path} (mode: {mode})...")
    test_data, obj_list = load_kaputt_test_data(
    ...
    if args.atomic_candidates_file is None:
        ...
        try:
            await generate_and_save_atomic_candidates(
                ...
                context=context,
                output_file=atomic_candidates_file,
                file_name=args.atomic_candidates_file,
            )
        except Exception as e:
    ...
    if args.concurrent == 1:
            ...
            results.append(
                {
                    ...
                    "item_identifier": sample["item_identifier"],
                    "defect": sample["defect"],
                    "major_defect": sample["major_defect"],
                    "defect_types": sample["defect_types"],
                    "query_image": sample["query_image"],
                    "full_query_image_path": sample["full_query_image_path"],
        ...
        tasks = [process_sample_with_semaphore(sample) for sample in test_data]
        results = []
    ...
    # Save results
    results_path = os.path.join(args.save_path, f"results_{mode}.json")
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="Evaluate agent_v1 on Kaputt dataset")
    parser.add_argument(
        "--parquet_path",
        type=str,
        default=DEFAULT_KAPUTT_PARQUET_PATH,
        help="Path to Kaputt parquet metadata file",
    )
    parser.add_argument(
        "--crop_root",
        type=str,
        default=DEFAULT_KAPUTT_CROP_ROOT,
        help="Root directory for cropped test images",
    )
    parser.add_argument(
        "--query_image_root",
        type=str,
        default=DEFAULT_KAPUTT_QUERY_IMAGE_ROOT,
        help="Root directory for full query images (overrides parquet paths)",
    )
    parser.add_argument(
        "--image_subdir",
        type=str,
        default="",
        help="Subdirectory under crop_root where cropped images are located (default: none)",
    )
    parser.add_argument(
        "--mode",
        type=str,
        default="both",
        choices=["any", "major", "both"],
        help="Evaluation mode: any (undamaged vs damaged), major (undamaged vs major), both (run both)",
    )
    parser.add_argument(
        "--save_path",
        type=str,
        default="./results/kaputt_agent_v1",
    ...
    if args.mode == "both":
        await evaluate_mode(args, "any")
        await evaluate_mode(args, "major")
    else:
        await evaluate_mode(args, args.mode)
...
# toy test:
# python evaluate_kaputt_v12.py --parquet_path ./data/kaputt/query-test.parquet --crop_root ./data/kaputt/query-crop/data/test/query-data/crop --query_image_root ./data/kaputt/query-image/data/test/query-data --save_path ./results/kaputt_test --mode both --max_samples 10 --device cuda --concurrent 2
